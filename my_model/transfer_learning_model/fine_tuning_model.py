...
from my_model.model_cnn import model_resnet_test as cnet
import random
import pandas as pd
...
def evaluate_accuracy(net, data_iter, epoch):
    ...
    labels = ['normal', 'insole_1layer', 'insole_2layer', 'insole_3layer']
...
def train(net, train_data, train_label, test_data, test_label, lr, weight_decay, num_epochs, batch_size, is_shuffle, lr_period, lr_decay, is_lr_decay=False):
    ...
    np.savetxt('D:/4.outcome/AFO_to_insole_outcome_cnn.csv', data_outcome, delimiter=',')

#tranfer_learning_model_modify
finetune_net = cnet.net
finetune_net.load_state_dict(torch.load('D:/1_study/programming_lauguage_learning/pythonProject/save_model_data/net/net_resCNN_state.pth'))
# nn.init.xavier_uniform_(finetune_net.fc.weight)


label_all, label_4 = [6, 7, 8, 3, 4, 5, 0, 1, 2], [1, 2, 3, 0]
window_size, step, num_kinds,  = 150, 150, 4
lr, num_epochs, batch_size, is_shuffle = 0.00001, 50, 128, True
weight_decay, lr_period, lr_decay, is_lr_decay = 0, 10, 0.9, False

# x = torch.rand(1, 36, 150)
# y = net(x)
# print(y.shape)

data_root = 'D:/3.data/3.conv_data/5.conv_data_train_test_insole_6'
train_data, train_label, test_data, test_label = data_processing(data_root, window_size, step, num_kinds, label_4)

train(finetune_net, train_data, train_label, test_data, test_label, lr, weight_decay, num_epochs, batch_size, is_shuffle, lr_period, lr_decay, is_lr_decay)
d2l.plt.show()