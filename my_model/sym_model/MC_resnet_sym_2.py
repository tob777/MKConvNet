...
class resnet(nn.Module):
    def __init__(self, input_channels, num_channels, use1x1_blk=False, strides=1):
        ...
        self.conv = nn.Sequential(
            nn.Conv1d(input_channels, num_channels, kernel_size=(3,), padding=1, stride=(strides,)), nn.BatchNorm1d(num_channels), nn.ReLU(),
            nn.Conv1d(num_channels, num_channels, kernel_size=(3,), padding=1), nn.BatchNorm1d(num_channels),
        )
        if use1x1_blk:
