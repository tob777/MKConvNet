...
import numpy as np
from my_model.sym_model import Multi_type_conv_kernel as cnet
...
def train(net, train_data, train_label, test_data, test_label, lr, weight_decay, num_epochs, batch_size, is_shuffle, lr_period, lr_decay, is_lr_decay=False):
    ...
    def _init_weight(m):
        if type(m) == nn.Linear or type(m) == nn.Conv1d or type(m) == nn.Conv2d or type(m) == nn.Conv3d:
    ...
    np.savetxt('D:/4.outcome/sym_cls/out.csv', data_outcome, delimiter=',')

net = cnet.net#torch.load('D:/1_study/programming_lauguage_learning/pythonProject/save_model_data/net/net_CNN.pth')
...
lr, num_epochs, batch_size, is_shuffle = 0.001, 306, 64, True
